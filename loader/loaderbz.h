...
*                                                                          *
* DESCRIPTION:                                                             *
*   BZ2 decompression routines for our loader system.                      *
*                                                                          *
...
struct S_DSTATE {
  ...
  bit32u tPos;
  int    k0;
  int    farF unzftab[256];
  int    nblock_used;
  int    farF cftab[257];
  int    farF cftabCopy[257];
  ...
  int    nInUse;
  bool   farF inUse[256];
  bool   farF inUse16[16];
  bit8u  farF seqToUnseq[256];

  /* for decoding the MTF values */
  bit8u  farF mtfa[MTFA_SIZE];
  int    farF mtfbase[256 / MTFL_SIZE];
  bit8u  farF selector[BZ_MAX_SELECTORS];
  bit8u  farF selectorMtf[BZ_MAX_SELECTORS];
  bit8u  farF len[BZ_N_GROUPS][BZ_MAX_ALPHA_SIZE];

  int    farF limit[BZ_N_GROUPS][BZ_MAX_ALPHA_SIZE];
  int    farF base[BZ_N_GROUPS][BZ_MAX_ALPHA_SIZE];
  int    farF perm[BZ_N_GROUPS][BZ_MAX_ALPHA_SIZE];
  int    farF minLens[BZ_N_GROUPS];
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// initialize the memory allocation block.  It is at DECOMP_MEM and
//  is 16 meg in size.
// We have a simple allocator.
// It has a 32-bit dword at the begining of each block that states
//  the size of the block.  If the size is zero, then the block is
//  not allocated.  We don't have to worry about fragmentation or
//  other, since this decompressor will allocate and free each block
//  before it frees a previous block.  ie.: this allocator is a simple
//  stack of memory where the 'top' most block gets used and freed
//  before the next 'top' most block gets freed and so on.

bit32u decomp_buf_loc = DECOMP_BUF;  // buffer where the compressed file is stored
bit32u decomp_mem_loc = DECOMP_MEM;  // buffer ptr to allocated buffers during decompression

void init_alloc() {
  (* (bit32u farF *) decomp_mem_loc) = 0x00000000;
}

void *bz_alloc(unsigned int size) {
  bit32u farF *p = (bit32u farF *) decomp_mem_loc;
  int i;
  
  while (*p)
    p = (bit32u farF *) ((bit32u) p + sizeof(bit32u) + *p);
  
  // make the size a multiple of 4
  size = (size + 3) & ~3;
  
  if ((size == 0) ||
      (((bit32u) p + size + 4) >= (decomp_mem_loc + DECOMP_BUF_SIZE))) {
    // great big error
    puts("\nError allocating the decoders memory...");
    freeze();
  }
  *p++ = size;  // store the size of this block, and move forward
  
  // now clear out that mem doing 4 more to mark the next position as zero
  size >>= 2;
  for (i=0; i<=size; i++)
    p[i] = 0;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// eax = passed memory pointer
// since we free the 'top' most memory only, we can simply mark the
//  size as zero.
void bz_free(void *buffer) {
  bit32u farF *p = (bit32u farF *) buffer;
  
  if ((bit32u) p > 0) {
    p--;  // back up to the size field
    
    if (((bit32u) p < decomp_mem_loc) || ((bit32u) p >= (decomp_mem_loc + DECOMP_BUF_SIZE))) {
      // great big error
      puts("\nError freeing the decoders memory...");
      freeze();
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void makeMaps_d(struct S_DSTATE farF *s) {
  int i;
  s->nInUse = 0;
  for (i = 0; i < 256; i++)
    if (s->inUse[i]) {
...
bool get_bits(struct S_DSTATE farF *s, bit32u *p, const int n) {
  while (1) {
    if (s->bsLive >= n) {
...
void BZ2_hbCreateDecodeTables(int farF *limit, int farF *base, int farF *perm, bit8u farF *length, 
...
/*---------------------------------------------------*/
int BZ2_decompress(struct S_DSTATE farF *s) {
  bit32u   uc;
  int      retVal;
  int      minLen, maxLen;
  ...
  switch (s->state) {
   ...
   case BZ_X_MAGIC_4: 
     ...
     s->tt = (bit32u *) bz_alloc(s->blockSize100k * 100000 * sizeof(int));
     if (s->tt == NULL) {
       retVal = BZ_MEM_ERROR;
       goto save_state_and_return;
     }
   ...
   case BZ_X_SELECTOR_2: 
    ...
    case BZ_X_MTF_1: 
      ...
      while (1) {
        put_progress(strm->total_in_lo32, strm->total_in_hi32);
        if (nextSym == EOB) 
          break;
...
int bz2_bzdecompressinit(struct S_BZ_STREAM *strm) {
  struct S_DSTATE farF *state = NULL;
  ...
  state = (struct S_DSTATE farF *) bz_alloc(sizeof(struct S_DSTATE));
  if (state == NULL)
    return BZ_MEM_ERROR;
...
bool unrle_obuf_to_output_fast(struct S_DSTATE farF *s) {
  bit8u k1;
  ...
  if (s->blockRandomised) {
    ...
    int          c_k0                 = s->k0;
    bit32u farF *c_tt                 = s->tt;
    bit32u       c_tPos               = s->tPos;
    bit8u farF  *cs_next_out          = s->strm->next_out;
...
int bz2_bzbufftobuffdecompress(struct S_BZ_STREAM *strm, void *targ, const int *dest_buf_size, void *src, const int len) {
  ...
  strm->next_in = (bit8u farF *) src;
  strm->next_out = (bit8u farF *) targ;
  strm->avail_in = len;
...
//
// return ecx
bool bz2_decompressor(void *targ, void *src, const int *size) {
  struct S_BZ_STREAM strm;
  int targ_size = 0x01000000;  // 1 meg
  int ret = 0;
  
  // initialize the progress proc
  // size is the sized of compressed data, not including the headers
  //  and what not.  We can either find this actual size or simply
  //  subtract an approximate header size of 4096 to make the progress
  //  bar reach 100%.  If we subtract too much, the progress bar will
  //  not go over 100% anyway.
  init_progress(*size - 4096);
  
  // initialize our simple allocator
  init_alloc();
  
  targ_size = 0x01000000;
