...
int BZ2_decompress(struct S_DSTATE *s) {
  ...
  switch (s->state) {
   ...
   case BZ_X_MAGIC_4: 
     ...
     s->tt = (bit32u *) AllocatePool(s->blockSize100k * 100000 * sizeof(int));
     if (s->tt == NULL) {
       retVal = BZ_MEM_ERROR;
       goto save_state_and_return;
     }
...
int bz2_bzdecompressinit(struct S_BZ_STREAM *strm) {
  ...
  state = (struct S_DSTATE *) AllocatePool(sizeof(struct S_DSTATE));
  if (state == NULL)
    return BZ_MEM_ERROR;
...
//
// return ecx
int bz2_decompressor(void *targ, void *src, const int *size) {
