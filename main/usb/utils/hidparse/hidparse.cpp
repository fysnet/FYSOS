/*
 *          File Name: hidparse.cpp  v1.50.00
 *  Last Modification: 28 Aug 2018
...
//  - no checking for C style comments, but does allow a comment
//    line to start with '#'
int hid_convert_to_bin(bit8u *report, FILE *fp) {
  ...
  while (1) {
    while (1) {
      ...
      // if we are at the end of the file, quit
      if (ch == EOF)
        return len;
      ...
      // if is a hexadecimal char, break
      if (isxdigit(ch))
        break;
    }
